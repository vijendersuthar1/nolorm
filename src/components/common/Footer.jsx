  ...
  return (
    <div className="max-w-[1170px] w-full mx-auto px-4 flex mt-[127px] gap-[87px] max-lg:flex-col max-lg:mt-[30px] max-lg:gap-10">
      ...
      <div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <ul className="text-[14px] text-[#253D4E] font-[open_Sans] font-normal leading-[21px] gap-2.5 flex flex-col   ">
                {items.links.map((link, i) => {
                  return <li className="hover:text-blue-500 ">{link.title}</li>;
                })}
              </ul>
            </div>
          ))}
        ...
        <div className="pt-10">
          <h2 className="text-[18px] text-[#130017] font-[quicksand] font-semibold ">
            Product tags
          </h2>
          <div className="flex gap-4 flex-wrap pt-3">
            {productTags.map((tag, index) => (
              <button
                key={index}
                className="text-[12px] text-[#253D4E] font-[quicksand] font-semibold bg-[#F2F0F0] rounded-xl py-1 px-[9.47px]">
                {tag}
              </button>
            ))}
          </div>
          <div className="flex justify-between items-center mt-[9px] border-t border-t-[#939393] max-md:flex-col max-md max-md:justify-center">
            <p className="text-[14px] text-[#253D4E] font-[lato] max-md:pt-5 ">
              Copyright © 2022 <span className="text-[#130017]">ThemeRage</span>
              . All Right Reserved.
            </p>
